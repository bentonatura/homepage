...
$\max\, c^\top x,\: Ax = b,\: x \geq 0,\: A \in \mathbb{R}^{m \times n}$ 
 in $O(n^{2.5} \log n\log (\bar{\chi}^\ast_A+n))$ iterations. Here, $\bar{\chi}^\ast\_A$
 is defined as the minimum value of the condition number $\bar{\chi}\_{A}$ achievable by a column rescaling.
This improves on the running time $O(n^{3.5} \log (\bar{\chi}\_A+n))$ of previous LLS methods. The result is achieved via a matroidal characterization of $\bar{\chi}^\ast\_A$. In particular, we give an efficient algorithm to find a column scaling $D$ satisfying $\bar{\chi}\_{AD} \leq n(\bar{\chi}^\ast)^3$.
The improved $O(n^{2.5})$ dependence is achieved by  a refined potential function based analysis for LLS algorithms in general. 

MarkDown has its **issues** if you _write_ this way.
